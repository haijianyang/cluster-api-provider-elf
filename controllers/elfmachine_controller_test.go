...
var _ = Describe("ElfMachineReconciler", func() {
	...
	Context("Reconcile ElfMachine VM", func() {
		...
		It("should create a new VM if none exists", func() {
			...
			Expect(logBuffer.String()).To(ContainSubstring("Connection status error detected for the ELF cluster"))
			expireELFScheduleVMError(machineContext, clusterConnectionKey)
