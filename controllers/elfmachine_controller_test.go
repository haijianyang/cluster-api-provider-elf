...
var _ = Describe("ElfMachineReconciler", func() {
	...
	Context("Reconcile an ElfMachine", func() {
		...
		It("should exit immediately on an error state", func() {
			elfMachine.Status.FailureReason = ptr.To(capierrors.CreateMachineError)
		...
		It("should requeue when node's healthy condition is unknown", func() {
			...
			result, err = reconciler.Reconcile(ctx, ctrl.Request{NamespacedName: elfMachineKey})
			Expect(err).ToNot(HaveOccurred())
			Expect(result.RequeueAfter).To(Equal(config.Cape.DefaultRequeueTimeout))
			Expect(logBuffer.String()).To(ContainSubstring(message))

			machine.Status.Conditions[0].LastTransitionTime = metav1.NewTime(time.Now().Add(-config.Task.VMPowerStatusCheckingDuration))
			ctrlMgrCtx = fake.NewControllerManagerContext(elfCluster, cluster, elfMachine, machine, secret, md)
			fake.InitOwnerReferences(ctx, ctrlMgrCtx, elfCluster, cluster, elfMachine, machine)
			reconciler = &ElfMachineReconciler{ControllerManagerContext: ctrlMgrCtx, NewVMService: mockNewVMService}
			result, err = reconciler.Reconcile(ctx, ctrl.Request{NamespacedName: elfMachineKey})
	...
	Context("Reconcile ElfMachine VM", func() {
		...
		It("should allow VM to be temporarily disconnected", func() {
			...
			mockVMService.EXPECT().GetVMNics(*vm.ID).Times(2).Return([]*models.VMNic{nic}, nil)
			mockVMService.EXPECT().GetVMPlacementGroup(gomock.Any()).Return(placementGroup, nil)
	...
	Context("Reconcile Join Placement Group", func() {
		...
		It("should add vm to the placement group", func() {
			...
			mockVMService.EXPECT().WaitTask(gomock.Any(), *task.ID, config.Task.WaitTaskTimeoutForPlacementGroupOperation, config.Task.WaitTaskInterval).Return(task, nil)
		...
		It("addVMsToPlacementGroup", func() {
			...
			klog.SetOutput(logBuffer)
			mockVMService.EXPECT().AddVMsToPlacementGroup(placementGroup, []string{*vm.ID}).Return(task, nil)
			mockVMService.EXPECT().WaitTask(gomock.Any(), *task.ID, config.Task.WaitTaskTimeoutForPlacementGroupOperation, config.Task.WaitTaskInterval).Return(nil, errors.New("xxx"))

			reconciler = &ElfMachineReconciler{ControllerManagerContext: ctrlMgrCtx, NewVMService: mockNewVMService}
			err = reconciler.addVMsToPlacementGroup(ctx, machineContext, placementGroup, []string{*vm.ID})
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring(fmt.Sprintf("failed to wait for placement group updating task to complete in %s: pgName %s, taskID %s", config.Task.WaitTaskTimeoutForPlacementGroupOperation, *placementGroup.Name, *task.ID)))
		})
	...
	Context("Reconcile ElfMachine providerID", func() {
		It("should set providerID to ElfMachine when VM is created", func() {
			...
			err := reconciler.reconcileProviderID(ctx, machineCtx, vm)
			Expect(err).To(BeZero())
	...
	Context("Reconcile ElfMachine network", func() {
		...
		It("should set ElfMachine to ready when VM network is ready", func() {
			...
			elfMachine = &infrav1.ElfMachine{}
			Expect(reconciler.Client.Get(ctx, elfMachineKey, elfMachine)).To(Succeed())
			Expect(elfMachine.Status.Network).To(HaveLen(2))
			Expect(elfMachine.Status.Network[0]).To(Equal(infrav1.NetworkStatus{
				IPAddrs:    []string{service.GetTowerString(nic0.IPAddress)},
				MACAddr:    service.GetTowerString(nic0.MacAddress),
				Gateway:    service.GetTowerString(nic0.Gateway),
				SubnetMask: service.GetTowerString(nic0.SubnetMask),
			}))
			Expect(elfMachine.Status.Network[1]).To(Equal(infrav1.NetworkStatus{
				IPAddrs:    []string{service.GetTowerString(nic1.IPAddress)},
				MACAddr:    service.GetTowerString(nic1.MacAddress),
				Gateway:    service.GetTowerString(nic1.Gateway),
				SubnetMask: service.GetTowerString(nic1.SubnetMask),
			}))
			Expect(elfMachine.Status.Addresses).To(HaveLen(2))
			Expect(elfMachine.Status.Addresses).To(ConsistOf(
				clusterv1.MachineAddress{
					Type:    clusterv1.MachineInternalIP,
					Address: service.GetTowerString(nic0.IPAddress),
				},
				clusterv1.MachineAddress{
					Type:    clusterv1.MachineInternalIP,
					Address: service.GetTowerString(nic1.IPAddress),
				},
			))
	...
	Context("Reconcile Placement Group", func() {
		...
		It("should makes sure that the placement group exist", func() {
			...
			Expect(result).To(BeZero())
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring(fmt.Sprintf("failed to wait for placement group creating task to complete in %s: pgName %s, taskID %s", config.Task.WaitTaskTimeoutForPlacementGroupOperation, placementGroupName, *withTaskVMPlacementGroup.TaskID)))
	...
	Context("reconcileLabels", func() {
		It("should add labels to the VM", func() {
			...
			mockVMService.EXPECT().UpsertLabel(gomock.Any(), gomock.Any()).Return(capeManagedLabel, nil)
			mockVMService.EXPECT().UpsertLabel(gomock.Any(), gomock.Any()).Return(namespaceLabel, nil)
			mockVMService.EXPECT().UpsertLabel(gomock.Any(), gomock.Any()).Return(clusterNameLabel, nil)
			...
			mockVMService.EXPECT().UpsertLabel(gomock.Any(), gomock.Any()).Return(namespaceLabel, nil)
			mockVMService.EXPECT().UpsertLabel(gomock.Any(), gomock.Any()).Return(clusterNameLabel, nil)
			mockVMService.EXPECT().AddLabelsToVM(*vm.ID, gomock.InAnyOrder([]string{*namespaceLabel.ID, *clusterNameLabel.ID})).Return(nil, nil)

			vm.Labels = []*models.NestedLabel{{Key: capeManagedLabel.Key, Value: capeManagedLabel.Value}}
			reconciler = &ElfMachineReconciler{ControllerManagerContext: ctrlMgrCtx, NewVMService: mockNewVMService}
			ok, err = reconciler.reconcileLabels(ctx, machineContext, vm)
			Expect(ok).To(BeTrue())
			Expect(err).ToNot(HaveOccurred())
