...
var _ = Describe("Lock GPU", func() {
	...
	It("lockVMGPUDevices", func() {
		gpuIDs := []string{gpuID}

		lockedVMGPUs := getLockedVMGPUDevices(clusterID, vmName)
		...
		Expect(lockVMGPUDevices(clusterID, vmName, hostID, gpuIDs)).To(BeTrue())
		lockedVMGPUs = getLockedVMGPUDevices(clusterID, vmName)
		...
		Expect(lockVMGPUDevices(clusterID, vmName, hostID, gpuIDs)).To(BeFalse())

		unlockVMGPUDevices(clusterID, vmName)
		lockedVMGPUs = getLockedVMGPUDevices(clusterID, vmName)
		Expect(lockedVMGPUs).To(BeNil())
		lockedClusterGPUIDs = getLockedClusterGPUIDs(clusterID)
		Expect(lockedClusterGPUIDs.Len()).To(Equal(0))
