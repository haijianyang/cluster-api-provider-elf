...
func (r *ElfMachineTemplateReconciler) reconcileCPResources(ctx *context.MachineTemplateContext) (bool, error) {
	...
	if ok, err := r.preflightChecksForCP(ctx, &kcp); err != nil || !ok {
		return ok, err
	}
...
func (r *ElfMachineTemplateReconciler) reconcileWorkerResourcesForMD(ctx *context.MachineTemplateContext, md *clusterv1.MachineDeployment) (bool, error) {
	...
	if ok := r.preflightChecksForWorker(ctx, md, updatingResourcesElfMachines); !ok {
		return ok, nil
	}

	maxSurge := getMaxSurge(md)
	toBeUpdatedElfMachines := machineutil.SelectFirstNElfMachines(needUpdatedResourcesElfMachines, maxSurge-len(updatingResourcesElfMachines))
	for i := 0; i < len(toBeUpdatedElfMachines); i++ {
		if err := markElfMachineToBeUpdatedResources(ctx, ctx.ElfMachineTemplate, toBeUpdatedElfMachines[i]); err != nil {
			return false, err
		}
	}
