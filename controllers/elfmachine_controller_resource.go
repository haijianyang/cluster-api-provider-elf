...
func (r *ElfMachineReconciler) reconcileVMResources(ctx *context.MachineContext, vm *models.VM) (bool, error) {
	...
	if restartKubeletJob == nil {
		...
		if err != nil {
			conditions.MarkFalse(ctx.ElfMachine, infrav1.ResourceHotUpdatedCondition, infrav1.RestartingKubeletFailedReason, clusterv1.ConditionSeverityInfo, err.Error())
		...
		conditions.MarkFalse(ctx.ElfMachine, infrav1.ResourceHotUpdatedCondition, infrav1.RestartingKubeletReason, clusterv1.ConditionSeverityInfo, "")
	...
	case agentv1.PhaseSucceeded:
		...
		conditions.MarkTrue(ctx.ElfMachine, infrav1.ResourceHotUpdatedCondition)
		ctx.Logger.Info("Restart kubelet succeeded", "hostAgentJob", restartKubeletJob.Name)
	case agentv1.PhaseFailed:
		annotationsutil.RemoveAnnotation(ctx.ElfMachine, infrav1.HostAgentJobNameAnnotation)
		conditions.MarkFalse(ctx.ElfMachine, infrav1.ResourceHotUpdatedCondition, infrav1.RestartingKubeletFailedReason, clusterv1.ConditionSeverityWarning, restartKubeletJob.Status.FailureMessage)
...
func (r *ElfMachineReconciler) updateVMResources(ctx *context.MachineContext, vm *models.VM) error {
	...
	if err != nil {
		conditions.MarkFalse(ctx.ElfMachine, infrav1.ResourceHotUpdatedCondition, infrav1.UpdatingResourceFailedReason, clusterv1.ConditionSeverityWarning, err.Error())

		return errors.Wrapf(err, "failed to trigger update for VM %s", ctx)
	}

	conditions.MarkFalse(ctx.ElfMachine, infrav1.ResourceHotUpdatedCondition, infrav1.UpdatingResourceReason, clusterv1.ConditionSeverityInfo, "")
