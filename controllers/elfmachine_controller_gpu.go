...
func (r *ElfMachineReconciler) selectHostAndGPUsForVM(ctx *context.MachineContext, preferredHostID string) (rethost *string, gpudevices []*service.GPUDeviceInfo, reterr error) {
	...
	// FindGPUDeviceInfos only returns the GPU that has been mounted on the virtual machine,
	// so you need to add the GPU that is not mounted on the VM.
	service.AggregateGPUDevicesToGPUDeviceInfos(gpuDeviceInfos, gpuDevices)
