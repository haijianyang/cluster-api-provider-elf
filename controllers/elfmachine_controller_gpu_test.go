...
var _ = Describe("ElfMachineReconciler-GPU", func() {
	...
	Context("reconcileVMGPUDevices", func() {
		gpuModel := "A16"
		...
		It("should not handle ElfMachine without GPU", func() {
			...
			ok, err := reconciler.reconcileVMGPUDevices(machineContext, vm)
	...
	Context("addVMGPUDevices", func() {
		...
		It("should migrate VM when current host does not have enough GPU devices", func() {
			...
			mockVMService.EXPECT().FindGPUDevices([]string{*host.ID}).Times(2).Return([]*models.GpuDevice{gpu}, nil)
			mockVMService.EXPECT().Migrate(*vm.ID, *host.ID).Return(withTaskVM, nil)

			machineContext := newMachineContext(ctrlContext, elfCluster, cluster, elfMachine, machine, mockVMService)
			reconciler := &ElfMachineReconciler{ControllerContext: ctrlContext, NewVMService: mockNewVMService}
			ok, err := reconciler.addVMGPUDevices(machineContext, vm)
			Expect(err).NotTo(HaveOccurred())
			Expect(ok).To(BeFalse())
			...
			unlockVMGPUDevices(elfCluster.Spec.Cluster, elfMachine.Name)
