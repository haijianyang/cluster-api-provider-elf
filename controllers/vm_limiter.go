...
func lockVMGPUDevices(clusterID, vmName, hostID string, gpuDeviceIDs []string) bool {
	...
	lockedClusterGPUIDs := getLockedClusterGPUDevicesWithoutLock(clusterID)
...
// getLockedClusterGPUDevices returns the locked GPU devices of the specified cluster.
func getLockedClusterGPUDevices(clusterID string) sets.Set[string] {
	gpuLock.Lock()
	defer gpuLock.Unlock()

	return getLockedClusterGPUDevicesWithoutLock(clusterID)
}
...
func getLockedClusterGPUDevicesWithoutLock(clusterID string) sets.Set[string] {
