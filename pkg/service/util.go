...
func HasGPUsCanNotBeUsedForVM(gpuDeviceInfos GPUDeviceInfos, elfMachine *infrav1.ElfMachine) bool {
	if elfMachine.RequiresGPUDevices() {
		filteredGPUDeviceInfos := gpuDeviceInfos.Filter(func(gpuInfo *GPUDeviceInfo) bool {
		...
		return filteredGPUDeviceInfos.Len() != gpuDeviceInfos.Len()
	}
	...
	for gpuID := range gpuDeviceInfos {
		...
		if count, ok := availableCountMap[gpuInfo.ID]; ok {
			availableCountMap[gpuInfo.VGPUTypeName] = count + gpuInfo.AvailableCount
		} else {
			availableCountMap[gpuInfo.VGPUTypeName] = gpuInfo.AvailableCount
		}
	}
