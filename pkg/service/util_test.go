...
func TestFilterGPUsCanNotBeUsedForVM(t *testing.T) {
	g := gomega.NewGomegaWithT(t)

	t.Run("should filter GPUs", func(t *testing.T) {
		g.Expect(FilterGPUsCanNotBeUsedForVM([]*models.GpuDevice{}, "vm")).To(gomega.BeEmpty())
		g.Expect(FilterGPUsCanNotBeUsedForVM([]*models.GpuDevice{{Vms: []*models.NestedVM{{ID: TowerString("id2"), Name: TowerString("vm2")}}}}, "vm")).To(gomega.BeEmpty())
		g.Expect(FilterGPUsCanNotBeUsedForVM([]*models.GpuDevice{{Vms: []*models.NestedVM{{ID: TowerString("id"), Name: TowerString("vm")}}}}, "vm")).To(gomega.HaveLen(1))
	})
}
