...
// New creates an empty Hosts.
func New() Hosts {
	return make(Hosts)
}

func FromHosts(hosts ...*models.Host) Hosts {
	ss := make(Hosts, len(hosts))
	ss.Insert(hosts...)
	return ss
}

// FromHostList creates a Hosts from the given host slice.
func FromHostList(hosts []*models.Host) Hosts {
	ss := make(Hosts, len(hosts))
	for i := range hosts {
		ss.Insert(hosts[i])
	}
